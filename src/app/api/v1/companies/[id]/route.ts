...
import { superAdminOrCompany, companyOnly } from '@/lib/middlewares/role.middleware'
...
export async function GET(
  ...
  try {
    ...
    if (!company) {
      return formatError(new Error('Company not found'))
    }
...
export async function PUT(
  ...
  try {
    ...
    if (userRole === RoleEnum.COMPANY && authResult.user.id !== id) {
      return formatError(new PermissionDeniedError('No tienes permisos para actualizar esta empresa. Solo puedes actualizar tu propia información.'))
    }
